...
time_summary$lineage_cor[time_summary$lineage_cor=="BA.5.5"] <- "BA.5"
time_summary$lineage_cor[time_summary$lineage_cor=="BA.5.2.1"] <- "BA.5"
time_summary$lineage_cor[time_summary$lineage_cor=="BA.5.1"] <- "BA.5"
...
Omicron_cases_freq <- subset(time_plot_relative_all, time_plot_relative_all$key =="BA.5" &
                                 time_plot_relative_all$date >= as.Date("2022-05-18")  &
                                 time_plot_relative_all$date <= as.Date("2022-06-25"))
...
model_plot <- ggplot(Omicron_cases_freq, aes(x=date, y =`Proportion of sequenced cases`)) +
    geom_point(alpha=0.2, color="#509c94")+
    geom_smooth(method = "glm", method.args = list(family=binomial(link = 'logit')),formula=y~x, fill="#509c94",aes(color="Predicted Prevalence"))+
    xlab("Date")+
    ylab("Percent of Sequenced Cases")+
    theme_minimal()+
    scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
    scale_color_manual(name = "",values =c("#509c94")) 
...
Ba2_cases <- subset(cases,cases$onset_date >= as.Date("2022-05-18") &
                        cases$onset_date <= as.Date("2022-06-25"))
Ba2_cases <-merge(Ba2_cases,fitted,by.x = "onset_date",by.y = "Omicron_cases_freq$date",all.x = TRUE)
Ba2_cases$'BA.2.12.1 cases' <- Ba2_cases$`Reported Cases`*Ba2_cases$`fitted(omicron_logit)`
...
cases_plot <- ggplot(Ba2_cases_melt, aes(x = `Onset Date`, y = Cases, fill=Type)) +
    ...
    xlab("Onset Date")+
    theme_minimal() +
    scale_fill_manual(name = "COVID-19 cases", labels = c("Reported Cases", "Cases of BA.2"),values =c("#D3D3D3","#509c94"))
...
ui <- fluidPage(
    ...
    mainPanel(fluidRow(splitLayout(cellWidths = c("70%", "30%"),
               h4("BA.5 Estimated Prevalence from 1st Detection"), h4("Current Prevalence of BA.5"))),
              fluidRow(splitLayout(cellWidths = c("70%", "30%"),
                          plotlyOutput("plot"), plotOutput("plot3"))),
              h4("Estimated Cases Attributed to BA.5", align='center'),
    ...
    output$plot3 <- renderPlot(
        ggplot(Omicron_cases_freq_most_recent,aes(x=as.Date(date)))+
            geom_boxplot(aes(lower=fitted-1.96*mod_se,upper=fitted+1.96*mod_se,middle=fitted,ymin=fitted-3*mod_se,ymax=fitted+3*mod_se),stat="identity",
                         fill="#96c4bf", color="#509c94") +
            ylab("Estimated Prevalence of BA.5") +
            xlab("Date")+
            theme_minimal()+
